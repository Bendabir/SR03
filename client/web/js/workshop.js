...
Workshop.getQueryParams = function workshop_getQueryParams() {
	...
	params.forEach(function(param){
		...
		param = param.split('=');
		res[param[0]] = param[1] || null;
	});

	return res;
};

Workshop.build = function(file, values, callback){
	Workshop.ajax({
		url: location.protocol + '//' + location.host + '/' + file,
		method:'GET' 
...
Workshop.app = function WorkshopApp(){
	this.scope = {};
	this.Router.parent = this;
	this.Controllers.parent = this;

	this.Router.init();
};

Workshop.app.prototype.__scope = function(elements){
...
Workshop.app.prototype.Router = {
	__controllers_location: '/controllers',
	__views_location: '/views',
	__styles_location: '/views/style'
};
Workshop.app.prototype.Router.setViews = function(viewslocation){
	this.__views_location = viewslocation;
	return this;
};
Workshop.app.prototype.Router.views =  Workshop.app.prototype.Router.setViews;

Workshop.app.prototype.Router.setControllers = function(controllerslocation){
	this.__controllers_location = controllerslocation;
	return this;
};
Workshop.app.prototype.Router.controllers =  Workshop.app.prototype.Router.setControllers;

Workshop.app.prototype.Router.setStyles = function(styleslocation){
	this.__styles_location = styleslocation;
	return this;
};
Workshop.app.prototype.Router.styles =  Workshop.app.prototype.Router.setStyles;

Workshop.app.prototype.Router.setDefaultContainer = function(container){
	this.__default_container = container;
	return this;
};

Workshop.app.prototype.Router.get = function(name, container){
	container = container || this.__default_container;

	return Workshop.ajax({
		method: 'GET',
		url: location.protocol + '//' + location.host + this.__views_location + '/' + name + '.html'
	}).then(res => {
		if(res.status < 300 && res.status > 200){
			var s = document.createElement('script');
			s.src = location.protocol + '//' + location.host + this.__controllers_location + '/' + name + '.js';

			var css = document.createElement('style');
			css.innerText = '@import url(\''+location.protocol + '//' + location.host + this.__styles_location + '/' + name + '.css'+'\');';
			...
			var e = new Error('Resource nor found: ' + name);
			e.response = res;
			throw e;
		}
	}).catch(e => {
...
Workshop.app.prototype.Router.__routes = {};

Workshop.app.prototype.Router.on = function(route){
	if(!(route instanceof Object)){
		throw new Error('Route is defined as {path:<String - route>, controller:<String - filename>}');
	}

	this.__routes[route.path] = route.controller;
};

Workshop.app.prototype.Router.init = function(){
	var parent = this;
	window.addEventListener('hashchange', function(){
		return parent.__load();
	});
};

Workshop.app.prototype.Router.__load = function(container){
	var hash = location.hash.replace(/#!/gi, '');
	var path = hash.split('?')[0];
	var params = hash.split('?')[1];
	var tree = path.split('/').filter(el=>{return el !== ''});

	var urlparams = {};
	var index = 0;

	var requested_route = null;
	var requested_file = null;

	console.log('Loading', path, tree);
	for(var route in this.__routes){
		if(route === path || route === (path + '/')){
			var subpaths = route.split('/').filter(e=>{return e !== ''});
			if(subpaths.length === tree.length){
				for(var subpath of subpaths){
					if(subpath[0] === ':'){
						urlparams[subpath[0].substr(1)] = tree[index];
					}
					index++;
				}
				requested_route = route;
				requested_file = this.__routes[route];
				break;
			}
		}
	}

	if(!requested_route || !requested_file){
		throw new Error('Route not found: ' + path);
	}

	this.parent.__current_route = {
		path:path, 
		params: urlparams,
		route: requested_route
	};

	return this.get(requested_file);
};

Workshop.app.prototype.go = function(path, params){
	path = '!' + path;
...
Workshop.app.prototype.Controllers = {};
Workshop.app.prototype.Controllers.new = function(controller){
	try {
		controller(this.parent, this.parent.scope, this.parent.__current_route);
	} catch(e) {
		console.error(e);
	}
};
